...
class BaseFeeder(data.Dataset):
    ...
    def read_video(self, index):
        ...
        def add_salt_and_pepper_noise(image, salt_prob, pepper_prob):
            ...
            noisy_image[salt_coords[0], salt_coords[1], :] = 255  # 白色（盐）

            # 添加椒噪声
            num_pepper = int(pepper_prob * total_pixels)
            pepper_coords = [np.random.randint(0, i - 1, num_pepper) for i in image.shape]
            noisy_image[pepper_coords[0], pepper_coords[1], :] = 0  # 黑色（椒）
        ...
        # images_with_noise = [
        #     add_salt_and_pepper_noise(cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB), salt_prob=0.0001,
        #                               pepper_prob=0.0001)
        #     for img_path in img_list
        # ]
        # return images_with_noise, label_list, fi

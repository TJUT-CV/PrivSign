...
class UpsampleModel(nn.Module):
    def __init__(self):
        ...
        self.Upsample = nn.ConvTranspose2d(64,3,kernel_size=4,stride=2,padding=1)
...
class mixModel(nn.Module):
    def __init__(self):
        ...
        self.mix = nn.Conv2d(64,64,kernel_size=1,stride=1,padding=0)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, kernels2=1):
        self.inplanes = 64
    ...
    def forward(self, x):
        ...
        #     """
        #     Visualizes each channel in a 3D tensor.
        #
        #     Args:
        #     - tensor: The input tensor with dimensions (channels, height, width).
        #
        #     Returns:
        #     - None
        #     """
        #     tensor_np = tensor.detach().cpu().numpy()
        #     tensor_np = tensor_np[:, :, 30:31, :, :]
        #     tensor_np = np.squeeze(tensor_np)
        #     num_channels, height, width = tensor_np.shape
        #     np.save('./withnpool_4.npy', tensor_np)
        #     max = tensor_np.max()
        #     min = tensor_np.min()
        #     tensor_np = (tensor_np - min) / (max - min)
        #     # Assuming num_channels is a perfect square for simplicity
        #     # mean = tensor_np.mean(dim=(0, 2, 3), keepdim=True)
        #     # std = tensor_np.std(dim=(0, 2, 3), keepdim=True)
        #
        #     ## norm
        #     # tensor_np = (tensor_np - mean) / (std + 1e-5)
        ...
        #         row = i // num_cols
        #         col = i % num_cols
        #         channel_data = tensor_np[i, :, :]
        #         axes[row, col].imshow(channel_data, cmap='gray', vmin=vmin, vmax=vmax)
        #         axes[row, col].set_title(f'Channel {i + 1}')
        #         axes[row, col].axis('off')
        #     plt.savefig('/visual/y6.jpg')
        #     plt.tight_layout()
        #     plt.show()
        N, C, T, H, W = x.size()
        ...
        # x = x.reshape(N, T, -1, 224, 224).permute(0, 2, 1, 3, 4)
        #
        # x = self.conv1(x)
        # x = self.bn1(x)
        # x = self.relu(x)
        # x = self.maxpool(x)
        # x = x.permute(0, 2, 1, 3, 4).reshape(-1, 64, 56, 56)
        # x_1 = self.gabor_layer_conv(x)
        # x = x_1 + x
        # x = x.reshape(N, T, -1, 56, 56).permute(0, 2, 1, 3, 4)
        # x = self.layer1(x)
